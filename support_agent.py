...
Extends Session 10 with human-in-the-loop approval gate.
interrupt_before=['github_tool_node'] pauses execution.
Human reviews draft before any write occurs.
Command(resume=...) resumes from the approval decision.
...
def run_session_verification() -> dict:
    """
    ┌─────────────────────────────────────────────────────────────┐
    │  SESSION 11 — VERIFICATION TEST                             │
    ├─────────────────────────────────────────────────────────────┤
    │  WHAT THIS TESTS:                                           │
    │  interrupt_before pauses before github_tool_node.           │
    │  snapshot.next contains 'github_tool_node' when paused.     │
    │  Approve resumes execution and creates the issue.           │
    │  Deny resumes execution and skips the issue.                │
    │  Edit & Approve creates issue with corrected draft.         │
    │                                                             │
    │  PASS CRITERIA:                                             │
    │  ✓ Critical ticket → graph pauses, snapshot.next populated │
    │  ✓ get_pending_approvals() returns the suspended thread    │
    │  ✓ Approve → github_issue_url set after resume            │
    │  ✓ Deny → github_issue_url empty, denial in notes         │
    │  ✓ Edit & Approve → URL set with corrected title          │
    │                                                             │
    │  WHAT A PASS PROVES:                                        │
    │  Human approval gate is operational.                        │
    │  No write happens without explicit human approval.          │
    │  All three approval actions work correctly.                 │
    │  Session 12 time travel can be added on top.                │
    └─────────────────────────────────────────────────────────────┘
    """
    ...
    # ── CHECK 1: Critical ticket pauses at github_tool_node ────────

    thread_1 = f"verify-s11-pause-{int(time.time())}"
    config_1  = {'configurable': {'thread_id': thread_1}}

    run_ticket(
        "Production API completely down. 500 errors across all regions "
        "for the past hour. Critical outage.",
        thread_id=thread_1,
    )

    snap1            = graph.get_state(config_1)
    paused_at_github = 'github_tool_node' in (snap1.next or [])
    draft_present    = bool(snap1.values.get('github_draft', {}).get('title'))

    check1_passed = paused_at_github and draft_present

    checks.append({
        'label':        'Critical ticket → graph pauses at github_tool_node',
        'passed':       check1_passed,
        'has_response': True,
        'note':         f"snapshot.next={list(snap1.next or [])} "
                        f"draft_present={draft_present}",
    })

    # ── CHECK 2: get_pending_approvals() returns thread ────────────

    pending          = get_pending_approvals()
    thread_in_pending = any(p.get('thread_id') == thread_1 for p in pending)

    check2_passed = thread_in_pending and len(pending) >= 1

    checks.append({
        'label':        'get_pending_approvals() returns suspended thread',
        'passed':       check2_passed,
        'has_response': True,
        'note':         f"pending_count={len(pending)} "
                        f"thread_found={thread_in_pending}",
    })

    # ── CHECK 3: Approve → github_issue_url set ───────────────────

    approve_result = approve_github_issue(thread_1)

    url_set    = bool(approve_result.get('url', '').strip())
    approved_ok = approve_result.get('success', False)

    check3_passed = approved_ok and url_set

    checks.append({
        'label':        'Approve → github_issue_url set after resume',
        'passed':       check3_passed,
        'has_response': True,
        'note':         f"success={approved_ok} url={approve_result.get('url','')[:40]}",
    })

    # ── CHECK 4: Deny → issue skipped, denial in notes ────────────

    thread_4 = f"verify-s11-deny-{int(time.time())}"
    config_4  = {'configurable': {'thread_id': thread_4}}

    run_ticket(
        "API returning 500 errors. Need immediate escalation.",
        thread_id=thread_4,
    )

    snap4 = graph.get_state(config_4)
    if 'github_tool_node' in (snap4.next or []):
        deny_result = deny_github_issue(thread_4)

        # After denial: url should be empty
        snap_after = graph.get_state(config_4)
        url_after  = snap_after.values.get('github_issue_url', '')
        notes      = snap_after.values.get('internal_notes', [])
        denial_logged = any(
            isinstance(n, dict) and 'denied' in n.get('finding', '').lower()
            for n in notes
        )

        check4_passed = (
            deny_result.get('success', False) and
            not url_after and
            denial_logged
        )

        checks.append({
            'label':        'Deny → issue skipped, denial logged in notes',
            'passed':       check4_passed,
            'has_response': True,
            'note':         f"url_empty={not url_after} "
                            f"denial_logged={denial_logged}",
        })
    else:
        checks.append({
            'label':        'Deny → issue skipped, denial logged in notes',
            'passed':       False,
            'has_response': False,
            'note':         'Thread did not pause — check interrupt_before config',
        })

    # ── CHECK 5: Edit & Approve → corrected title in URL ──────────

    thread_5 = f"verify-s11-edit-{int(time.time())}"
    config_5  = {'configurable': {'thread_id': thread_5}}

    run_ticket(
        "Critical database connection failures affecting all customers.",
        thread_id=thread_5,
    )

    snap5 = graph.get_state(config_5)
    if 'github_tool_node' in (snap5.next or []):

        edit_result = edit_and_approve_github_issue(
            thread_id=thread_5,
            edited_title='[EDITED] Database connection pool exhausted — P0',
            edited_body='Corrected diagnosis: connection pool exhausted, '
                        'not API error. Root cause: insufficient pool size.',
        )

        url_set5   = bool(edit_result.get('url', '').strip())
        edit_ok    = edit_result.get('success', False)
        was_edited = edit_result.get('edited', False)

        check5_passed = edit_ok and url_set5 and was_edited

        checks.append({
            'label':        'Edit & Approve → issue created with corrected title',
            'passed':       check5_passed,
            'has_response': True,
            'note':         f"success={edit_ok} edited={was_edited} "
                            f"url={edit_result.get('url','')[:40]}",
        })
    else:
        checks.append({
            'label':        'Edit & Approve → issue created with corrected title',
...
def run_cli_tests():
    """Runs all Session 11 test cases when file is executed directly."""

    print("\n" + "█" * 64)
    print("█  ENTERPRISE AI SUPPORT PLATFORM — SESSION 11 OF 12       █")
    print("█  Interruptions & Breakpoints                             █")
    print("█" * 64)

    import time as _time
    _ts = int(_time.time())

    # TEST 1 — Verify interrupt fires
    print(f"\n{'─' * 60}")
    print("TEST 1 — Verify interrupt fires")
    thread_1 = "test-s11-pause"
    ticket1  = "Production API completely down. Critical outage."
    print(f"TICKET: {ticket1}")
    print("EXPECTED: graph pauses before github_tool_node")
    run_ticket(ticket1, thread_id=thread_1)
    config_1 = {'configurable': {'thread_id': thread_1}}
    snap1 = graph.get_state(config_1)
    paused = 'github_tool_node' in (snap1.next or [])
    draft1 = snap1.values.get('github_draft', {})
    url_before = snap1.values.get('github_issue_url', '')
    print(f"Graph paused:                  {paused}")
    print(f"Draft title:                   {draft1.get('title','(none)')[:60]}")
    print(f"Issue URL (should be empty):   '{url_before}'")
    passed1 = paused
    print(f"Status: {'✅ PASS' if passed1 else '❌ FAIL'}")

    # TEST 2 — Check pending approvals
    print(f"\n{'─' * 60}")
    print("TEST 2 — Check pending approvals")
    pending = get_pending_approvals()
    print(f"Pending approvals: {len(pending)}")
    for p in pending:
        print(f"  thread={p['thread_id'][-8:]} title='{p.get('title','')[:40]}'")
    passed2 = any(p.get('thread_id') == thread_1 for p in pending)
    print(f"Status: {'✅ PASS' if passed2 else '❌ FAIL'}")

    # TEST 3 — Approve
    print(f"\n{'─' * 60}")
    print("TEST 3 — Approve")
    result3 = approve_github_issue(thread_1)
    print(f"Approved:  {result3.get('success')}")
    print(f"URL:       {result3.get('url','(none)')[:70]}")
    snap3 = graph.get_state(config_1)
    completed = snap3.next == () or snap3.next == []
    print(f"Graph completed: {completed}")
    passed3 = result3.get('success') and bool(result3.get('url','').strip())
    print(f"Status: {'✅ PASS' if passed3 else '❌ FAIL'}")

    # TEST 4 — Deny
    print(f"\n{'─' * 60}")
    print("TEST 4 — Deny")
    thread_4 = f"test-s11-deny-{_ts}"
    ticket4  = "API returning 500 errors. Needs engineering escalation."
    print(f"TICKET: {ticket4}")
    run_ticket(ticket4, thread_id=thread_4)
    config_4 = {'configurable': {'thread_id': thread_4}}
    snap4 = graph.get_state(config_4)
    if 'github_tool_node' in (snap4.next or []):
        deny_result = deny_github_issue(thread_4)
        snap_after4 = graph.get_state(config_4)
        url_after4  = snap_after4.values.get('github_issue_url', '')
        notes4      = snap_after4.values.get('internal_notes', [])
        print(f"URL empty: {not bool(url_after4)}")
        denial_note = next(
            (n for n in notes4 if isinstance(n, dict) and
             'denied' in n.get('finding', '').lower()),
            None,
        )
        print(f"Denial note: {denial_note.get('finding','') if denial_note else '(not found)'}")
        passed4 = deny_result.get('success') and not bool(url_after4) and bool(denial_note)
    else:
        print("Thread did not pause — check interrupt_before config")
        passed4 = False
    print(f"Status: {'✅ PASS' if passed4 else '❌ FAIL'}")

    # TEST 5 — Edit & Approve
    print(f"\n{'─' * 60}")
    print("TEST 5 — Edit & Approve")
    thread_5 = f"test-s11-edit-{_ts}"
    ticket5  = "Critical DB failures affecting all customers."
    print(f"TICKET: {ticket5}")
    run_ticket(ticket5, thread_id=thread_5)
    result5 = edit_and_approve_github_issue(
        thread_5,
        edited_title='[EDITED] DB pool exhausted — P0 incident',
        edited_body='Root cause: connection pool. Not API error.',
    )
    print(f"Edited & approved: {result5.get('success')}")
    print(f"URL: {result5.get('url','(none)')[:70]}")
    passed5 = result5.get('success') and bool(result5.get('url','').strip())
    ...
    print(f"\n{'═' * 64}")
    print(f"SESSION 11 COMPLETE — {verification['summary']}")
